...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of RICEs
    ...
    // Bonus muliplier for early rice makers.
    ...
    function pendingRice(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accRicePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 riceReward = multiplier.mul(ricePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        rice.mint(devaddr, riceReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                safeRiceTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
    ...
    // Safe rice transfer function, just in case if rounding error causes pool to not have enough RICEs.
